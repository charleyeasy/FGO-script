    ...
    while True:
        ...
        try:
            round = auto("menu.png", support,clothes,servant_class, int(apple_count), apple, device, int(
                recover_time) * 60, run_time=int(1), ver=ver)
 
            instr = decoder.decode(codelist)
            instr.pop()  # 移除最後一個元素 不執行最後的finish_battle 直接結束
            print("執行次數:1，遊戲內專用")
            print("指令列表:")
            # 首先計算最大的序號寬度
            ...
            for i, instruction in enumerate(instr, 1):                      
                cleaned_instruction = instruction.replace("round.", "", 1) # 移除開頭的 "round."
